package losiba
{
[CLASS1900]   import flash.events.EventDispatcher;
   ...
   public class Executer extends EventDispatcher
   {
      ...
      public static const roviq:String = "returned";

      public static const zisu:String = "ExeValue|((com.junkbyte.console.core::)?Executer)";

      public static function rivaju(param1:Object, param2:String, param3:Object=null) : * {
         var _loc4_:Executer = new Executer();
         _loc4_.wiki(param3);
         return _loc4_.exec(param1,param2);
      }

      private static const kusysawyh:String = "#";

      private var rebucuby:Array;

      private var muli:Boolean;

      private var fitozi;

      private var vepyfybi;

      private var nuhunaz:Object;

      private var jofyfa:Array;

      public var jatij:Boolean;

      public function get returned() : * {
         return this.vepyfybi;
      }
      ...
      public function wiki(param1:Object) : void {
         var _loc2_:* = false;
         var _loc3_:* = true;
         this.nuhunaz=param1;
         return;
      ...
      public function telo(param1:Array) : void {
         var _loc2_:* = false;
         var _loc3_:* = true;
         this.jofyfa=param1;
         return;
      ...
      public function exec(param1:*, param2:String) : * {
         ...
         else
         {
            ...
            if(!this.nuhunaz)
            {
               this.nuhunaz=new Object();
            }
            if(!this.jofyfa)
            {
               this.jofyfa=new Array();
            }
            this.lurates(str);
            this.reset();
            return this.vepyfybi;
      ...
      private function lurates(param1:String) : void {
         var _loc14_:* = false;
         var _loc15_:* = true;
         ...
         while(_loc3_!=null)
         {
            ...
            param1=this.heseh(param1,new ExeValue(_loc10_),_loc3_.index+_loc8_,_loc3_.index+_loc9_+1);
      ...
      private function jafemelu(param1:String) : * {
         var _loc16_:* = false;
         var _loc17_:* = true;
         var _loc3_:* = 0;
         var _loc4_:* = 0;
         ...
         var param1:String = this.rytuvab(param1);
         ...
         while(_loc2_>=0)
         {
            ...
            if(param1.substring(_loc2_+1,_loc3_).search(new RegExp("\\w"))>=0)
            {
               ...
               if(_loc7_)
               {
                  ...
                  param1=this.heseh(param1,new ExeValue(_loc10_),_loc2_+1,_loc5_);
                  for (_loc11_ in _loc10_)
                  {
                     _loc10_[_loc11_]=this.kusyz(this.rytuvab(_loc10_[_loc11_])).value;
                  }
               }
               else
               {
                  _loc12_=new ExeValue(_loc12_);
                  param1=this.heseh(param1,_loc12_,_loc2_,_loc5_+1);
                  _loc12_.setValue(this.kusyz(this.rytuvab(_loc6_)).value);
               }
            }
            _loc2_=param1.lastIndexOf("(",_loc2_-1);
         }
         this.vepyfybi=this.kusyz(param1).value;
         if((this.vepyfybi)&&(this.jatij))
         {
            _loc13_=typeof this.vepyfybi;
            if(_loc13_=="object"||_loc13_=="xml")
            {
               this.fitozi=this.vepyfybi;
            }
         }
         dispatchEvent(new Event(Event.COMPLETE));
         return this.vepyfybi;
      ...
      private function heseh(param1:String, param2:*, param3:int, param4:int) : String {
         var _loc5_:* = true;
         var _loc6_:* = false;
         var param1:String = param1.substring(0,param3)+kusysawyh+this.rebucuby.length+param1.substring(param4);
         this.rebucuby.push(param2);
         return param1;
      }

      private function kusyz(param1:String) : ExeValue {
         var _loc16_:* = true;
         var _loc17_:* = false;
         var _loc7_:String = null;
         ...
         else
         {
            ...
            while(_loc3_!=null)
            {
               ...
               if(_loc3_==null)
               {
                  ...
                  _loc4_.push(this.rytuvab(_loc13_));
         ...
         while(_loc6_<_loc5_)
         {
            _loc4_[_loc6_]=this.lomavic(_loc4_[_loc6_]);
            _loc6_=_loc6_+2;
         }
         ...
         while(_loc6_<_loc5_)
         {
            ...
            {
               _loc8_=this.qetono(_loc4_[_loc6_-1],_loc7_,_loc4_[_loc6_+1]);
         ...
         while(_loc6_<_loc5_)
         {
            ...
            if(_loc7_.replace(_loc9_,"")=="")
            {
               ...
               _loc8_=this.qetono(_loc15_,_loc7_,_loc10_);
      ...
      private function lomavic(param1:String) : ExeValue {
         var _loc5_:* = false;
         var _loc6_:* = true;
         ...
         var v:ExeValue = new ExeValue(this.fitozi);
         if(str.indexOf("new ")==0)
         {
            ...
            newobj=this.rohonisab(newstr.substring(4));
            str=this.heseh(str,new ExeValue(newobj),0,newstr.length);
         }
         var reg:RegExp = new RegExp("\\.|\\(","g");
         var result:Object = reg.exec(str);
         if(result==null||!isNaN(Number(str)))
         {
            return this.bibos(str,this.fitozi);
         }
         ...
         if(firstparts.length>0)
         {
            ...
            else
            {
               ...
               try
               {
                  def=getDefinitionByName(this.rytuvab(classstr));
                  havemore=str.length>classstr.length;
                  str=this.heseh(str,new ExeValue(def),0,classstr.length);
                  if(havemore)
                  {
                     reg.lastIndex=0;
                     result=reg.exec(str);
         ...
         while(result!=null)
         {
            ...
            basestr=this.rytuvab(str.substring(previndex,index));
            newv=previndex==0?this.bibos(basestr,v.value):new ExeValue(v.value,basestr);
            if(isFun)
            {
               newbase=newv.value;
               closeindex=str.indexOf(")",index);
               paramstr=str.substring(index+1,closeindex);
               paramstr=this.rytuvab(paramstr);
               params=[];
               if(paramstr)
               {
                  params=this.bibos(paramstr).value;
               }
      ...
      private function bibos(param1:String, param2:*=null) : ExeValue {
         var _loc5_:* = true;
         var _loc6_:* = false;
         var v:ExeValue = null;
            ...
            else
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     {
                        if(str.indexOf(kusysawyh)==0)
                        {
                           vv=this.rebucuby[str.substring(kusysawyh.length)];
                           v.obj=vv.value;
                        }
                        else
                        {
                           if(str.charAt(0)=="$")
                           {
                              ...
                              {
                                 if(this.nuhunaz is Laqajuvo)
                                 {
                                    v.obj=Laqajuvo(this.nuhunaz).Vinac(key);
                                 }
                                 else
                                 {
                                    v.obj=this.nuhunaz[key];
                                 }
                              }
                              return v;
                           }
      ...
      private function qetono(param1:ExeValue, param2:String, param3:ExeValue) : * {
         var _loc5_:* = true;
         var _loc6_:* = false;
         switch(null)
         {
      ...
      private function rohonisab(param1:String) : * {
         var _loc9_:* = false;
         var _loc10_:* = true;
         var _loc5_:* = 0;
         var _loc6_:String = null;
         ...
         _loc3_=this.rytuvab(_loc3_);
         var _loc4_:* = this.bibos(_loc3_).value;
         if(_loc2_>0)
         {
            _loc5_=param1.indexOf(")",_loc2_);
            _loc6_=param1.substring(_loc2_+1,_loc5_);
            _loc6_=this.rytuvab(_loc6_);
            _loc7_=[];
            if(_loc6_)
            {
               _loc7_=this.bibos(_loc6_).value;
            }
      ...
      private function rytuvab(param1:String) : String {
         var _loc3_:* = false;
         var _loc4_:* = true;
