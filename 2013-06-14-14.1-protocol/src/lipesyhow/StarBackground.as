package lipesyhow
{
   ...
   public class StarBackground extends Background
   {
      public function StarBackground() {
         var _loc2_:* = true;
         var _loc3_:* = false;
         this.tevureboz=new Vector.<Star>();
      ...
      public var tevureboz:Vector.<Star>;
      ...
      override public function draw(param1:Qawosiwi, param2:int) : void {
         ...
         for each (_loc3_ in this.tevureboz)
         {
      ...
      private function katuha() : void {
         var _loc6_:* = false;
         var _loc7_:* = true;
         var _loc3_:Star = null;
         var _loc1_:Fufudol = AssetLibrary.bugu("stars");
         var _loc2_:Star = new Star(Math.random()*1000-500,Math.random()*1000-500,4*(0.5+0.5*Math.random()),_loc1_.gisydi[int(_loc1_.gisydi.length*Math.random())]);
         for each (_loc3_ in this.tevureboz)
         {
            if(PointUtil.nybomof(_loc2_.x_,_loc2_.y_,_loc3_.x_,_loc3_.y_)<3)
            {
               return;
            }
         }
         this.tevureboz.push(_loc2_);
         return;
      }
   }
