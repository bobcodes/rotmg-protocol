package com.company.assembleegameclient.objects
{
   ...
   public class Wall extends GameObject
   {
      ...
      public function Wall(param1:XML) {
         ...
         super(param1);
         kyjon=false;
         var _loc2_:Sirador = ObjectLibrary.pupeheb[objectType_];
         this.bovas=_loc2_.getTexture(0);
         return;
      }

      private static const qewilaler:Vector.<Number> = new Vector.<Number>(12);

      private static const cegy:Vector.<int> = new Vector.<int>(4);

      private static const lerup:Vector.<int> = new Vector.<int>(4);

      public var faces_:Vector.<Face3D>;

      private var sykidymy:Face3D = null;

      private var bovas:BitmapData = null;
      ...
      override public function setObjectId(param1:int) : void {
         ...
         var _loc2_:Sirador = ObjectLibrary.pupeheb[objectType_];
         this.bovas=_loc2_.getTexture(param1);
         return;
      }

      override public function getColor() : uint {
         return BitmapUtil.hitav(this.bovas);
      }
      ...
      override public function draw(param1:Vector.<IGraphicsData>, param2:Wypyj, param3:int) : void {
         ...
         if(jizatoqym!=null)
         {
            _loc6_=jizatoqym.getTexture(param3);
            if(_loc6_!=null)
            {
               _loc4_=_loc6_;
            }
         ...
         while(_loc5_<this.faces_.length)
         {
            ...
            _loc8_=map_.lookupSquare(x_+cegy[_loc5_],y_+lerup[_loc5_]);
            if(_loc8_==null||_loc8_.texture_==null||!(_loc8_==null)&&_loc8_.obj_ is Wall&&!_loc8_.obj_.hojucope)
            {
               _loc7_.tehymoha=true;
            }
            else
            {
               _loc7_.tehymoha=false;
               if(jizatoqym!=null)
               {
                  _loc7_.senarofuk(_loc4_);
               }
            }
            _loc7_.draw(param1,param2);
            _loc5_++;
         }
         this.sykidymy.draw(param1,param2);
         return;
      }
      ...
      public function vojyr() : void {
         ...
         this.sykidymy=new Face3D(this.bovas,_loc3_,qewilaler,false,true);
         this.sykidymy.bitmapFill_.repeat=true;
         this.nafakus(_loc1_,_loc2_,1,_loc1_+1,_loc2_,1);
         this.nafakus(_loc1_+1,_loc2_,1,_loc1_+1,_loc2_+1,1);
         this.nafakus(_loc1_+1,_loc2_+1,1,_loc1_,_loc2_+1,1);
         this.nafakus(_loc1_,_loc2_+1,1,_loc1_,_loc2_,1);
         return;
      }

      private function nafakus(param1:Number, param2:Number, param3:Number, param4:Number, param5:Number, param6:Number) : void {
         var _loc9_:* = true;
         var _loc10_:* = false;
         ...
         var _loc8_:Face3D = new Face3D(texture_,_loc7_,qewilaler,true,true);
